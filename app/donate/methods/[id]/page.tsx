...
export default function DonationMethodEditPage() {
  ...
  const loadDonationMethod = async () => {
    try {
      setLoadingData(true);
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`/api/donation-methods/${params.id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!response.ok) throw new Error('فشل جلب طريقة التبرع');
      ...
      message.error('فشل جلب طريقة التبرع: ' + error.message);
      router.push('/donate');
    } finally {
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'فشل تحديث طريقة التبرع');
      }
      
      message.success('تم تحديث طريقة التبرع بنجاح');
      router.push('/donate');
    } catch (error: any) {
      message.error('فشل تحديث طريقة التبرع: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout>
      <h1 className="page-title">تعديل طريقة تبرع</h1>

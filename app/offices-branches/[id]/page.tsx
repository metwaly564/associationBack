...
export default function OfficeBranchEditPage() {
  ...
  const loadOffice = async () => {
    try {
      ...
      const response = await fetch(`/api/offices-branches/${params.id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!response.ok) throw new Error('فشل جلب المكتب/الفرع');
      ...
      message.error('فشل جلب المكتب/الفرع: ' + error.message);
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'فشل تحديث المكتب/الفرع');
      }
      
      message.success('تم تحديث المكتب/الفرع بنجاح');
      router.push('/offices-branches');
    } catch (error: any) {
      message.error('فشل تحديث المكتب/الفرع: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout>
      <h1 className="page-title">تعديل مكتب/فرع</h1>

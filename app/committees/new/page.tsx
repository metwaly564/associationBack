...
export default function CommitteeCreatePage() {
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (!response.ok) {
        ...
        const msg = error.error || 'فشل إنشاء اللجنة';
        const details = error.details ? `: ${error.details}` : '';
        throw new Error(msg + details);
      }
      
      const data = await response.json();
      const committeeId = data.committee.id;
      
      // Update members and tasks
      if (values.members?.length > 0 || values.tasks?.length > 0) {
      ...
      message.success('تم إنشاء اللجنة بنجاح');
      router.push('/committees');
    } catch (error: any) {
      message.error('فشل إنشاء اللجنة: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout>
      <h1 className="page-title">إضافة لجنة جديدة</h1>
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item name="type" label="النوع" initialValue="permanent">
          <Select>

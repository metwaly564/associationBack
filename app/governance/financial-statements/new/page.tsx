...
export default function FinancialStatementCreatePage() {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'فشل إنشاء القائمة المالية');
      }
      
      message.success('تم إنشاء القائمة المالية بنجاح');
      router.push('/governance/financial-statements');
    } catch (error: any) {
      message.error('فشل إنشاء القائمة المالية: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout>
      <h1 className="page-title">إضافة قائمة مالية</h1>

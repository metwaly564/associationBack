...
export async function GET(
  ...
  try {
    ...
    const result = await pool.query('SELECT * FROM project_categories WHERE id = $1', [
      id,
    ]);
    ...
    const result = await pool.query(
      `UPDATE project_categories 
       SET name = $1, slug = $2, description = $3, color = $4, 
           icon_name = $5, order_index = $6, is_active = $7, updated_at = NOW()
       WHERE id = $8
       RETURNING *`,
      [name, slug, description || null, color, icon_name || null, order_index, is_active, id]
    );
...
export async function DELETE(
  ...
  try {
    ...
    if (parseInt(projectsCheck.rows[0].count) > 0) {
      return NextResponse.json(
        { error: 'لا يمكن حذف التصنيف لأنه مستخدم في مشاريع موجودة' },
        { status: 400 }
      );
    }

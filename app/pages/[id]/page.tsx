...
export default function StaticPageEditPage() {
  ...
  const onFinish = async (values: any) => {
    try {
      await api.updatePage(params.id, {
        title: values.title,
        slug: values.slug,
        type: values.type,
      ...
      message.success('تم حفظ الصفحة بنجاح');
      router.push('/pages');
    } catch (error: any) {
      message.error('فشل حفظ الصفحة: ' + error.message);
    }
  };

  return (
    <AdminLayout>
      <h1 className="page-title">تعديل صفحة ثابتة</h1>
        ...
        <Tabs
          ...
          items={[
            ...
            },
            {
              ...
              children: (
                <>
                  <Form.Item name="showInMenu" valuePropName="checked" label="إظهار في القائمة">
                ...
                <>
                  <Form.Item name="status" label="الحالة">
                    <Radio.Group>
                      ...
                      <Radio value="published">منشورة</Radio>
                    </Radio.Group>
                  </Form.Item>
                </>
              ),

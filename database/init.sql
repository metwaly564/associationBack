-- Script لإنشاء قاعدة البيانات والجداول
-- استخدم: psql -U postgres -f database/init.sql

-- إنشاء قاعدة البيانات (إذا لم تكن موجودة)
SELECT 'CREATE DATABASE association_db'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'association_db')\gexec

-- الاتصال بقاعدة البيانات
\c association_db

-- إنشاء Extension للـ UUID إذا لم تكن موجودة
...
-- Governance: Operational Plan table
...
-- Governance: Financial Statements table
...
CREATE INDEX IF NOT EXISTS idx_association_members_category ON association_members(category);
